...
import pl.ddl.todo.customer.CustomerView;
import pl.ddl.todo.customer.CustomerViewImpl;
import pl.ddl.todo.login.LoginView;
import pl.ddl.todo.login.UserLoggedInEvent;
    ...
    protected void init(VaadinRequest request) {
        ...
        if (!isLoggedIn()) {
            ViewMenuUI.getMenu()
                      .setVisible(false);
            ViewMenuUI.getMenu()
                      .navigateTo(LoginView.ID);
        } else {
            if (getNavigator().getState()
                              .isEmpty()) {
                ViewMenuUI.getMenu()
                          .navigateTo(CustomerViewImpl.class);
            }
        }
    }
