...
import pl.ddl.todo.AbstractView;
...
public class LoginViewImpl extends AbstractView<LoginViewPresenter> implements LoginView {
    ...
    private final Button.ClickListener loginClickListener = new Button.ClickListener() {
        ...
        public void buttonClick(ClickEvent event) {
            getPresenter().onLoginPressed(username.getValue(), password.getValue());
        }
    };

    private final TextField username = new MTextField().withInputPrompt("User name");
    ...
    public LoginViewImpl() {
        ...
        loginPanel.setContent(new MVerticalLayout(username, password, login).withAlign(login,
                Alignment.BOTTOM_RIGHT));

        setCompositionRoot(
                new MVerticalLayout(loginPanel).withAlign(loginPanel, Alignment.MIDDLE_CENTER)
                                               .withFullHeight());
    }

    @Override
    public void showInvalidLoginNotification() {
        Notification.show("Login failed", "Invalid credentials", Type.TRAY_NOTIFICATION);
    }
